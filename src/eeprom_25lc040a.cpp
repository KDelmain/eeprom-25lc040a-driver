...
namespace {
/* mask for the lower 4 bits inside a 16-byte page */
constexpr uint16_t PAGE_ALIGNMENT_MASK =
    Eeprom25LC040A::PAGE_SIZE_BYTES - 1;
}
...
bool Eeprom25LC040A::ReadSequential(uint16_t address, uint8_t* buffer,
                                    uint16_t length)
{
    ...
    const uint8_t a8 = static_cast<uint8_t>(
        (address & internal::ADDRESS_A8_MASK) >> internal::ADDRESS_A8_BIT_POS);
    const uint8_t opcode = static_cast<uint8_t>(
        instruction::READ_MEMORY | (a8 << internal::OPCODE_A8_BIT_POS));
...
bool Eeprom25LC040A::WritePage(uint16_t address,
                               const uint8_t* data,
                               uint8_t length)
{
    ...
    const uint8_t a8 = static_cast<uint8_t>(
        (address & internal::ADDRESS_A8_MASK) >> internal::ADDRESS_A8_BIT_POS);
    const uint8_t opcode = static_cast<uint8_t>(
        instruction::WRITE_MEMORY | (a8 << internal::OPCODE_A8_BIT_POS));
    const uint8_t addr_low = static_cast<uint8_t>(address & 0xFF);

    std::array<uint8_t, Eeprom25LC040A::PAGE_SIZE_BYTES + 2> tx = {};
    tx[0] = opcode;
    tx[1] = addr_low;
    std::copy_n(data, length, tx.begin() + 2);

    Select();
    bool ok = spi_interface_.Transmit(tx.data(),
...
bool Eeprom25LC040A::WriteSequential(uint16_t address,
                                     const uint8_t* data,
                                     uint16_t length)
{
    ...
    while (remaining > 0)
    {
        uint8_t space_in_page = static_cast<uint8_t>(
            PAGE_SIZE_BYTES - (current_address % PAGE_SIZE_BYTES));
        uint8_t chunk = (remaining < space_in_page)
                            ? static_cast<uint8_t>(remaining)
                            : space_in_page;

        if (!WritePage(current_address, data_ptr, chunk))
            return false;
...
bool Eeprom25LC040A::SetBlockProtect(BlockProtect level)
{
    uint8_t sr = 0;
    if (!ReadStatus(sr))
        return false;

    sr &= static_cast<uint8_t>(
        ~(status::BLOCK_PROTECT_BIT_0 | status::BLOCK_PROTECT_BIT_1));
    sr |= static_cast<uint8_t>(level) << 2;

    return WriteStatus(sr);
}

void Eeprom25LC040A::EnableHardwareWriteProtect(bool enable)
{
    if (enable)
        write_protect_pin_.SetLow();
    else
        write_protect_pin_.SetHigh();
}

void Eeprom25LC040A::Pause()
{
    hold_pin_.SetLow();
}

void Eeprom25LC040A::Resume()
{
    hold_pin_.SetHigh();
}
...
bool Eeprom25LC040A::WriteDisable()
{
    const uint8_t cmd = instruction::WRITE_DISABLE;
    Select();
    bool ok = spi_interface_.Transmit(&cmd, 1);
    Deselect();
    return ok;
...
{
    using namespace eeprom_25lc040a::timing;

    uint8_t  status  = 0;
    uint32_t elapsed = 0;
    const uint32_t timeout = WRITE_CYCLE_MAX_MS + WRITE_CYCLE_GUARD_MS;
    ...
    while (elapsed < timeout)
    {
        ...
        {
            /* Clear WEL explicitly for safety */
            WriteDisable();
            return true;
        }
...
{
    return (address & ~PAGE_ALIGNMENT_MASK) ==
           ((address + length - 1) & ~PAGE_ALIGNMENT_MASK);
}
...
bool Eeprom25LC040A::IsWithinMemoryBounds(uint16_t address,
                                          uint16_t length) const
{
    return (address + length) <= MEMORY_SIZE_BYTES;
}

bool Eeprom25LC040A::IsValidPageWrite(uint16_t address,
                                      uint8_t length) const
{
    return IsSamePage(address, length) &&
           IsWithinMemoryBounds(address, length);
}
